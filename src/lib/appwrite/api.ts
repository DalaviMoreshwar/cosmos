...
// Auth functionality of creatring a new user
...
// delete saved post functionality
...
export async function updatePost(post: IUpdatePost) {
  const hasFileToUpdate = post.file.length > 0; // check if file has to updated before updating post
  try {
    ...
    // if yes, update the image instance
    if (hasFileToUpdate) {
      // Upload file to appwrite storage
      const uploadedFile = await uploadFile(post.file[0]);
